...
import { ArrowLeft, CalendarDays, Eye, EyeOff, LockKeyhole, Mail, UserRound } from "lucide-react";
		...
		<main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-secondary/15 via-background to-primary/10 px-4 py-10">
			<section className="w-full max-w-lg rounded-3xl border border-border/70 bg-card p-6 shadow-xl sm:p-10">
				...
				<div className="mb-8 flex items-start gap-4">
					<div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><CalendarDays className="h-6 w-6" /></div>
					<div><h1 className="text-3xl font-bold tracking-tight">Crear cuenta</h1><p className="mt-1 text-muted-foreground">Regístrate para acceder a EventUTN.</p></div>
				</div>
				<form className="space-y-5" onSubmit={handleSubmit}>
					<label className="block space-y-2 text-sm font-medium">Nombre completo<div className="relative"><UserRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input name="fullName" type="text" placeholder="Ana Rodríguez" className="h-11 pl-10" required minLength={3} autoComplete="name" /></div></label>
					<label className="block space-y-2 text-sm font-medium">Correo electrónico<div className="relative"><Mail className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input name="email" type="email" placeholder="tu@correo.com" className="h-11 pl-10" required autoComplete="email" /></div></label>
					...
					<label className="block space-y-2 text-sm font-medium">Repetir contraseña<Input name="confirmPassword" type={showPassword ? "text" : "password"} placeholder="Repite tu contraseña" className="h-11" required minLength={6} autoComplete="new-password" /></label>
